  ...
  head: () => ({
    meta: [
      { title: "RM — Discord moderation, security and community bot" },
      {
        name: "description",
        content:
          "RM protects your Discord server with moderation, AutoMod, anti-nuke and anti-raid, all managed from one dashboard.",
      },
      { property: "og:title", content: "RM — Discord security bot" },
      {
        property: "og:description",
        content:
          "Moderation, AutoMod, anti-nuke, anti-raid, tickets, leveling and economy in one bot.",
      },
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="mx-auto max-w-6xl px-6 pb-24">
        <section className="py-16">
          ...
          >
            ...
            {status?.latencyMs != null && online ? ` · ${status.latencyMs}ms` : ""}
          </span>
          <h1 className="mt-6 max-w-3xl text-5xl font-semibold leading-tight tracking-tight">
            One bot to moderate, protect and grow your Discord server.
          </h1>
          <p className="mt-5 max-w-2xl text-lg text-muted-foreground">
            RM answers to <code className="text-primary">rm!</code> and{" "}
            <code className="text-primary">rm?</code>, custom prefixes, mentions
            and slash commands — with anti-nuke, anti-raid and AutoMod running
            in the background.
          </p>
          <div className="mt-8 flex flex-wrap gap-3">
            ...
            >
              Browse {COMMANDS.length} commands
            </a>
          </div>
        </section>
      ...
      <footer className="border-t border-border py-8 text-center text-sm text-muted-foreground">
        RM {status?.version ? `v${status.version}` : ""} — self-hosted Discord bot
      </footer>
    </div>
  );
}
