...
export async function getLocalUserFromSession() { const session = await getRMSession(); if (session.data.authProvider !== "local" || !session.data.userId) return null; const store = await getStore(); return store.users[session.data.userId] ?? null; }
