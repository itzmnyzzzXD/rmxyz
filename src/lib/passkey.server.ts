...
export async function getRegistrationOptions() {
  ...
  const options = await generateRegistrationOptions({ rpName: RP_NAME, rpID: siteHost(), userName: user.username, userID: new TextEncoder().encode(user.id), attestationType: "none", excludeCredentials: existing.map((x) => ({ id: x.id, transports: x.transports as never })), authenticatorSelection: { residentKey: "required", userVerification: "required", authenticatorAttachment: "platform" }, supportedAlgorithmIDs: [-7, -257] });
...
export async function verifyPasskeyRegistration(response: unknown, name = "This device") {
  ...
  const verification = await verifyRegistrationResponse({ response: response as Parameters<typeof verifyRegistrationResponse>[0]["response"], expectedChallenge: pending.challenge, expectedOrigin: siteUrl(), expectedRPID: siteHost(), requireUserVerification: true, supportedAlgorithmIDs: [-7, -257] });
...
export async function getAuthenticationOptions() {
  ...
  const options = await generateAuthenticationOptions({ rpID: siteHost(), userVerification: "required", allowCredentials: Object.values(store.credentials).map((x) => ({ id: x.id, transports: x.transports as never })) });
...
export async function verifyPasskeyAuthentication(response: unknown, challengeId: string) {
  ...
  const verification = await verifyAuthenticationResponse({ response: response as Parameters<typeof verifyAuthenticationResponse>[0]["response"], expectedChallenge: pending.challenge, expectedOrigin: siteUrl(), expectedRPID: siteHost(), requireUserVerification: true, credential: { id: credential.id, publicKey: new Uint8Array(credential.publicKey), counter: credential.counter, transports: credential.transports as never } satisfies WebAuthnCredential });
